...
def get_embedding(
    model, contexts: List[str], context_words: List[List[str]], do_concat_tokens=True
):
    
    '''Get the embeddings for the context kwics using the specified model.
    
    Args: 
        model: The embedding model to use
        contexts: List of context strings
        context_words: List of lists of context words (verb-noun pairs)
        do_concat_tokens: Whether to concatenate token embeddings
    '''
    embeddings = [] # List to store embeddings for each context

    for cxt, cxt_words in zip(contexts, context_words): # iterate through contexts and their corresponding context words
        ...
        def _match_words(sentence, verb, noun):
            ...
            for word1_id in range(len(sentence)):
                word1 = sentence[word1_id].text # get the text of the current token
                if word1 == verb:
        ...
        if result is None: # if we can't find the verb and noun in the sentence, print a warning and skip this context
            ...
            _embeddings = [sentence[word1_id].embedding, sentence[word2_id].embedding] # get the embeddings of the matched verb and noun tokens

        # SANITY CHECK: Ensure that we have embeddings for exactly the number of context words expected
        assert len(_embeddings) == len(
            cxt_words
        ), f"Mismatch in lengths: {_embeddings} vs {cxt_words}" # should be 2 for our current use case (verb and noun)

        # Stack the embeddings of the current context words
        embeddings.append(torch.stack(_embeddings)) # shape: (num_context_words=2, embedding_dim) e.g., (2, 384) for SBERT with verb and noun embeddings

    # Stack the embeddings across all contexts and take the mean across them
    word_embeddings = torch.stack(embeddings, dim=0) # (num_contexts, num_context_words=2, embedding_dim)
    word_embeddings = word_embeddings.mean(dim=0) # (num_context_words=2, embedding_dim) - average across contexts to get a single embedding per context word

    if do_concat_tokens:
        # If concatenation is requested, flatten the tensor
        word_tokens_output = word_embeddings.reshape(-1) # (num_context_words * embedding_dim,) e.g., (768,) for SBERT with verb and noun embeddings concatenated
    else:
        # Otherwise, take the mean across the embeddings. This will give us a single embedding 
        # that represents the combined meaning of the verb and noun in the context. We won't be able to separate verb and noun information.
        word_tokens_output = torch.mean(word_embeddings, dim=0) # (embedding_dim,) e.g., (384,) for SBERT with verb and noun embeddings averaged
